...
size_t idFileSystemLocal::CurlWriteFunction( void *ptr, size_t size, size_t nmemb, void *stream ) {
	...
	#ifdef _WIN32
		auto filePtr = static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr();
		auto fileno = _fileno(filePtr);
		return _write( fileno, ptr, size * nmemb );
	#else
	...
	while( 1 ) {
		...
		if ( bgl->opcode == DLTYPE_FILE ) {
			// use the low level read function, because fread may allocate memory
				...
				auto filePtr = static_cast<idFile_Permanent*>(bgl->f)->GetFilePtr();
				auto fileno = _fileno(filePtr);
				_read( fileno, bgl->file.buffer, bgl->file.length );
			#else
